...
/** Saves to NewsletterSubscriber — included in daily digest (~12:00 PM IST) + welcome email. */
    ...
    try {
      ...
      if (res.ok && looksSuccess) {
        return {
          ok: true,
          message:
            "You're in! Check your inbox — daily 3-story digest ~12:00 PM IST (unsubscribe link in every email).",
        };
      }
      lastMessage =
        data.error ||
